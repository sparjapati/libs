...
- **Always add client-facing utility functions** — whenever a new type, sealed class, or data structure is introduced that clients must construct or return (e.g. `RowResult`, `ExtraColumns`), add companion object factory functions and/or extension functions that make the common cases ergonomic. Clients should never need to construct internals manually. Example: `RowResult.withExtras(...)`, `RowResult.noExtras()`, `Map<SpreadsheetRow, T>.allSaved()`, `Map<SpreadsheetRow, T>.toRowResults { }` added alongside `RowResult`.
- **Invoke methods on known non-null receivers** — when an operation can be expressed with either a known non-null receiver or a potentially null receiver, prefer the non-null receiver. This avoids accidental `NullPointerException`s and makes intent explicit. Example: `"ACTIVE".equals(status)` not `status.equals("ACTIVE")`. In Kotlin, prefer null-safe operators and `==` over direct `equals()` calls.

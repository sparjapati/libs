...
class VendorApiLoggingInterceptorTest {
    ...
    @Test fun `logs successful response`() {
        VendorApiLoggingInterceptor(logSink = sink, requestIdProvider = { "req-abc" })
            .intercept(chain(200))
    ...
    @Test fun `logs failed response (4xx) as not success`() {
        VendorApiLoggingInterceptor(sink).intercept(chain(404))
    ...
    @Test fun `logs thrown exception and rethrows`() {
        val interceptor = VendorApiLoggingInterceptor(sink)
    ...
    @Test fun `logs binary response body as 'binary response only'`() {
        VendorApiLoggingInterceptor(sink).intercept(chain(contentType = "application/octet-stream"))
    ...
    @Test fun `logs binary request body as 'binary response only'`() {
        VendorApiLoggingInterceptor(sink).intercept(chain(requestContentType = "application/octet-stream"))
        assertEquals(BINARY_BODY_PLACEHOLDER, capturedLog.captured.requestBody)
    }

    @Test fun `saves raw unmasked headers`() {
        VendorApiLoggingInterceptor(sink).intercept(
    ...
    @Test fun `does not log when no TraceableApi annotation`() {
        ...
        VendorApiLoggingInterceptor(mockSink).intercept(mockChain)
